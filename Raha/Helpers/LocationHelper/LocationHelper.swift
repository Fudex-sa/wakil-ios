...
class LocationHelper: NSObject {
    ...
    /* currentLocation */
    var location: Publisher<CLLocation> = .init()
    var degree: CLLocationCoordinate2D? {
        return location.publisher?.value?.coordinate
    }
    var lat: Double? {
        return location.publisher?.value?.coordinate.latitude
    }
    var lng: Double? {
        return location.publisher?.value?.coordinate.longitude
    }
    override init() {
        super.init()
...
extension LocationHelper: CLLocationManagerDelegate {
    private func locationPermession() -> Bool {
    ...
    public func currentLocation() {
        if !authroize(.locationWhenInUse) {
            return access(.locationWhenInUse)
        }
        self.locationManager.requestWhenInUseAuthorization()
        self.locationManager.requestAlwaysAuthorization()
        if CLLocationManager.locationServicesEnabled() {
...
extension LocationHelper: Permission {
    public func reload() {
        if !authroize(.locationWhenInUse) {
            onErrorLocation?()
        } else {
