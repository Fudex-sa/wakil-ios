...
//
//
//  Created by M.abdu on 2022/07/03.
//
...
public class VideoCompressor {
    ...
    public enum VideoQuality: Equatable {
        ...
        /// Scale video size proportionally, not large than 720p and
        /// reduce fps and bit rate if need.
        case mediumHighQuality
    ...
    public func compressVideo(_ url: URL, quality: VideoQuality = .mediumHighQuality, completion: @escaping (Result<URL, Error>) -> Void) {
    ...
    func getVideoBitrateWithQuality(_ quality: VideoQuality, originalBitrate: Float) -> Float {
        ...
        if originalBitrate < targetBitrate {
            ...
            case .mediumQuality:
                ...
                targetBitrate = max(targetBitrate, Float(Self.minimumVideoBitrate))
            case .mediumHighQuality:
                targetBitrate = originalBitrate/3
                targetBitrate = max(targetBitrate, Float(Self.minimumVideoBitrate))
            case .highQuality:
