...
final class Localizer: NSObject {
    ...
    class var instance: Localizer {
        if Static.instance == nil {
            Static.instance = Localizer()
        }
        return Static.instance!
    }
    
    var language: Publisher<Languages> = .init()
    
    
    override init() {
        super.init()
        language.listen(on: { [weak self] value in
            self?.set(language: value ?? .english)
        })
    }
    ...
    /**
     ...
     - returns: void
     */
    private func set(language: Languages) {
        DispatchQueue.main.async {
            let lang = Localizer.getSelectedLanguages().contains(language.rawValue) ? language.rawValue : Localizer.defaultSign
            guard lang != Localizer.current.rawValue else { return }
