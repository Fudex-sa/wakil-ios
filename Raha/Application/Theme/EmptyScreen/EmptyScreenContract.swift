...
extension EmptyScreenContract where Self: BaseController {
    func showEmptyScreen(for top: CGFloat = 140, for bottom: CGFloat = 0, title: String? = "Empty screen".localized, body: String? = "This page is blank and has no content So now please browse the another page") {
        hideEmptyScreen()
        emptyScreen = EmptyScreen()
        emptyScreen.titleLbl.text = title?.localized
        //emptyScreen.bodyLbl.text = body?.localized
        emptyScreen.container.backgroundColor = .clear
